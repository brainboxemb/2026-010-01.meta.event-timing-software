...
Software ideas, possible requirements, technology choices, architecture options, and unresolved questions must first be captured in `docs/brainstorm.md`.
...
Before ending a substantial work session, update `docs/agent-plan.md` if progress changed and keep `docs/handoff.md` usable as a stable entry point for the next session. The handoff should point to sources of truth rather than attempting to duplicate the complete current state.
