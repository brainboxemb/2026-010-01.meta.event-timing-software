#!/usr/bin/env python3
"""Generate A4 screen SVGs for SIP planning.

The existing planning generator remains the source of the A2/A3 print views.
This companion generator provides compact A4 screen views from the same SIP and
validated YAML planning sources:
...
- blocks use less empty space so text remains readable on a monitor;
...
def render_roadmap_page(
    ...
    svg_text(parts, MARGIN_MM, 10.5, ["Software Implementation Planning — roadmap"], 5.2, anchor="start", weight="bold")
    svg_text(
        parts,
        MARGIN_MM,
        17.5,
...
def render_step_a4(board: dict, step: Step, path: Path) -> None:
    parts = [
        f'<svg xmlns="http://www.w3.org/2000/svg" width="{A4_P_W_MM}mm" height="{A4_P_H_MM}mm" '
        f'viewBox="0 0 {A4_P_W_MM} {A4_P_H_MM}">',
        '<rect width="100%" height="100%" fill="white"/>',
    ]
    svg_text(parts, MARGIN_MM, 10.0, wrap(f"SIP Step {step.number} — {step.title}", 82, 2), 4.0, anchor="start", weight="bold")
    svg_text(
        parts,
        MARGIN_MM,
        17.0,
    ...
    if total_lane_h > available_h:
        raise SystemExit(
            f"Step {step.number} A4 board does not fit: needs {total_lane_h:.1f} mm, has {available_h:.1f} mm"
        )
...
def append_readme(out_dir: Path, roadmap_pages: List[Path], step_paths: List[Tuple[int, Path]]) -> None:
    ...
    lines = [
        ...
        "## A4 screen views",
        "",
        "These SVGs are compact monitor-oriented presentations of the same SIP/YAML planning sources. Roadmap pages contain at most four milestones; detailed step boards are a drill-down of the same milestone plan. A2/A3 outputs remain available for print.",
        "",
        "### Roadmap pages",
        "",
    ]
    for index, path in enumerate(roadmap_pages, start=1):
        lines.append(f"- [A4 roadmap {index}](./a4/{path.name})")
    lines.extend(["", "### Step boards", ""])
    for number, path in step_paths:
        lines.append(f"- [Step {number} A4](./a4/{path.name})")
    lines.append("")
...
def main() -> None:
    ...
    for page_index, group in enumerate(groups):
        path = a4_dir / f"sip-roadmap-a4-{page_index+1}.svg"
    ...
    for number, board in sorted(boards.items()):
        ...
        path = a4_dir / f"step-{number:02d}-a4.svg"
        render_step_a4(board, step, path)
