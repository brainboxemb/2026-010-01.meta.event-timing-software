...
11. any source documents or references explicitly named by the active plan step
...
Treat the pull request as the detailed work/evidence record for the active step. Treat `docs/02-agent-plan.md` as the longer-lived coordination plan. Treat the SDP/SIP as software-evolution planning and the SDE as the common development-environment/workflow definition. Do not assume that a branch name or an old handoff statement is current without checking GitHub state.

Then continue only with the currently active agent-plan step unless later work is required to correct the plan itself.

## Current software-item numbering direction

```text
01  Headless Timing Application
02  Desktop GUI Application
03  Browser/iPad Operator Application (candidate)
```

The software-item number is stable across that item's SRD/SAD/SDD documents. It is not a document sequence number.

System-owned interfaces are documented through IDDs and may be referenced as applicable requirements by software-item SRDs.

## Reusable handoff message

> Work from `2026-010-01.meta.event-timing-software` as the coordination and research source.
>
> Read `AGENTS.md`, `docs/01-handoff.md`, `docs/02-agent-plan.md`, `docs/00-brainstorm.md`, `docs/10-SDP-software-development-plan.md`, `docs/11-SIP-software-implementation-planning.md`, `docs/12-SDE-software-development-environment.md`, `docs/30-SSAD-software-system-architecture.md`, and the relevant software-item design documents.
>
...
>
> Use the SDP for staged software direction, the SIP for the implementation sequence, and the SDE for GitHub/tooling/AI/repository workflow. Keep implementation details and evidence in the corresponding implementation PR.
>
