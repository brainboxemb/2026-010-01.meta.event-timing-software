...
Treat the pull request as the detailed work/evidence record for the active step. Treat `docs/02-agent-plan.md` as the longer-lived **AP-*** coordination plan. Treat `docs/03-domain-baseline.md` as working domain knowledge/terminology rather than formal requirements. Treat the SDP/SIP as software-evolution planning, the SDE as the common development-environment/workflow definition, and the SVP as the common verification strategy. Do not assume that a branch name or an old handoff statement is current without checking GitHub state.
